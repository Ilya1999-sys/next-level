...
1. Horizontal card rows fill 100% width without empty right tracks. Cards in a row share equal height. Top hero cards (`.tournament-hero`) have a fact card (`.hero-fact`) placed to their right on large screens (`.row-hero-split`, ~65–70% hero / ~30–35% fact) to eliminate large empty gaps between text and circles; on ≤900px, the hero fact moves to the bottom of the page (`order: 99`). Single text-only facts never stretch alone next to a tall story/chart card; stack 2, 3, or 4 facts vertically (`col-stack`) instead (Barcelona has 4 facts; Real Madrid 15 Cups has 3 facts; Mbappe player-stats has 2 facts on left of Real Madrid cups; Euro 2008 banner matches 2 facts). Charts (`MixChart`) and dot grids (`DotGrid`) scale proportionally (Real Madrid cups dot grid scales up `clamp(24px, 2.8vw, 34px)` and pins to the bottom-left corner) to fill leftover vertical height inside tall cards. Zidane/stat circle cards hug height with circles side-by-side in one row (`flex-wrap: nowrap`). Favorites screen cards use wider ~340–360px columns so circles stay horizontal (3–4 cards per row on desktop). Tournaments match row includes a 3rd card on desktop. On ≤900px, main story cards stay in reading order while fact stacks move to the bottom. Inner card padding is `--size-card-pad` (20px). No chrome fade/flicker on route change.
