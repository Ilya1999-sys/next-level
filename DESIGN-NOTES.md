...
1. Horizontal card rows fill 100% width. Do not leave an empty track to the right of the last card; add one or two sibling cards that fit, or grow the last-row items. Cards in a row share one height. Text-only facts: title top-left, number + description bottom-left (`margin-top: auto`). Title + fact + chart: number stays on the title with `--size-fact-title-gap`; leftover height goes to the chart. Title + fact + description (Player files): same bottom-left pin, padding `--size-card-pad` (`space/l`). Number + copy sit in one row while copy fits in two lines; three or more lines stack under the number. A story or chart card sits left of a stack of two facts so a single fact never becomes a large empty field. Zidane-style circle cards hug: circles in one horizontal row at the bottom, no empty band under them. On ≤900px, Home facts move to the bottom of the page. Inner inset on every card is `--size-card-pad`. Do not fade the chrome on route change.
